 ...
 * ========================================================= */
export function initResumePdfExport({ ui, locale } = {}) {
  const BLUE  = '#3E9BCD';
  const DARK  = '#1f1f1f';
  const MUTED = '#6b6b6b';
  const CHIP  = '#999999';
  ...
  btns.forEach(btn => btn.addEventListener('click', async (e) => {
    ...
    if (typeof pdfMake === 'undefined') {
      b.textContent = ui?.actions?.pdfmakeNotLoaded || 'pdfmake not loaded';
      setTimeout(() => { b.textContent = originalText; }, 2000);
      return;
    }
    try {
      b.disabled = true;
      b.textContent = ui?.actions?.buildingPdf || 'building...';
      ...
      b.textContent = ui?.actions?.failed || 'failed — check console';
      setTimeout(() => { b.textContent = originalText; b.disabled = false; }, 2500);
    }
  }));

  async function fetchCv(currentLocale) {
    const path = DATA_PATHS.cv(currentLocale);
  ...
  function metaLine(start, end, location, margin = [0, 0, 0, 0]) {
    return {
      columns: [
        { width: 'auto', text: `${start} - ${end}`, fontSize: 7.5, color: MUTED },
        { width: 12, text: '' },
        {
          width: 'auto',
          columns: [
            { width: 8, svg: ICONS.pin, fit: [7, 7], margin: [0, 0.5, 0, 0] },
            { width: 'auto', text: location, fontSize: 7.5, color: MUTED, margin: [3, 0, 0, 0] }
          ]
        }
      ],
      margin
  ...
  function workBlock(job, isLast) {
    const stack = [
      ...
      metaLine(fmtDate(job.startDate), fmtDate(job.endDate), job.location || '', [0, 0, 0, 2])
    ];
  ...
  function educationBlock(edu) {
    return {
      stack: [
        ...
        metaLine(fmtDate(edu.startDate), fmtDate(edu.endDate), edu.location || '')
      ],
      margin: [0, 0, 0, 6]
    };
  }
